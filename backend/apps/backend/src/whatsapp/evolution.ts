...
export async function sendOrderCreatedWhatsApp({
  ...
  if (!baseUrl || !apiKey || !instance) {
    logger.warn("WhatsApp notifications skipped: missing EVOLUTION_API_BASE_URL / EVOLUTION_API_KEY / WHATSAPP_INSTANCE");
    return;
  }
  ...
  if (!number) {
    logger.warn(`WhatsApp notifications skipped: missing/invalid phone for order ${order?.id || ""}`);
    return;
  }
  ...
  try {
    ...
    logger.info(`WhatsApp order confirmation sent for order ${order?.id || ""} to ${number}`);
  } catch (e: any) {
    logger.error(`WhatsApp send failed for order ${order?.id || ""}: ${e?.message || e}`);
  }
}
