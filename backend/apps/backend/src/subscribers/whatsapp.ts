...
export async function sendOrderUpdatedWhatsApp({
  order,
  newStatus,
}: {
  order: any;
  ...
  console.log(`=== WHATSAPP SEND START (UPDATED) ===`);
  console.log(`EVOLUTION_API_BASE_URL: ${baseUrl}`);
  console.log(`EVOLUTION_API_KEY: ${apiKey ? "SET" : "NOT SET"}`);
  console.log(`WHATSAPP_INSTANCE: ${instance}`);
  ...
  // Use template for order updated
  const text = getWhatsAppTemplate('order_updated', { order, newStatus });
  ...
  try {
    ...
    console.log(`WhatsApp order updated notification sent for order ${order?.id || ""} to ${number}`);
    console.log(`=== WHATSAPP SEND END (SUCCESS) ===`);
  } catch (e: any) {
    console.error(`WhatsApp send failed for order ${order?.id || ""}: ${e?.message || e}`);
    console.error(`Error details:`, e);
