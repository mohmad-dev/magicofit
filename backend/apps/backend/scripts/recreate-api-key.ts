...
export default async function recreateApiKey({ container }: { container: MedusaContainer }) {
  const apiKeyModuleService = container.resolve("api_key")
  const salesChannelModuleService = container.resolve("sales_channel")
  
  // Delete all existing publishable keys
  const existingKeys = await apiKeyModuleService.listApiKeys({ type: "publishable" })
  for (const key of existingKeys) {
    await apiKeyModuleService.deleteApiKeys(key.id)
  }
  console.log(`🗑️ Deleted ${existingKeys.length} existing publishable keys`)
